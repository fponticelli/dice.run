...
Main.prototype = $extend(doom_html_Component.prototype,{
	render: function() {
		...
		if(_g8[1] == 1) {
			...
			var n = _g8[3];
			var s = _g8[2];
			children6 = new view_ProbabilitiesView({ expression : n, parsed : e1, probabilities : haxe_ds_Option.None, selected : haxe_ds_Option.None}).asNode();
		} else {
...
view_Editable.prototype = $extend(doom_html_Component.prototype,{
	render: function() {
		...
		var value3 = doom_core__$AttributeValue_AttributeValue_$Impl_$.fromString(this.props.value);
		if(__map_reserved["value"] != null) {
			_g.setReserved("value",value3);
		} else {
			_g.h["value"] = value3;
		}
	...
	,didMount: function() {
		view_Editable.selectRange(this.node);
	}
	,didUpdate: function() {
		view_Editable.selectRange(this.node);
	}
...
view_ProbabilitiesView.getFromLocalStorage = function(expr) {
	var storage = window.localStorage;
	var item = storage.getItem(view_ProbabilitiesView.STORAGE_PREFIX + expr);
	if(null == item) {
		return haxe_ds_Option.None;
	}
	var ob = JSON.parse(item);
	return haxe_ds_Option.Some(ProbabilitiesResult.fromObject(ob));
};
...
view_ProbabilitiesView.prototype = $extend(doom_html_Component.prototype,{
	...
	,renderProbabilities: function(probabilities) {
	...
	,render: function() {
		var maybe;
		var _g = this.props.probabilities;
		if(_g[1] == 1) {
			maybe = view_ProbabilitiesView.getFromLocalStorage(this.props.expression);
		} else {
			var some = _g;
			maybe = some;
		}
		switch(maybe[1]) {
		case 0:
			var probabilities = maybe[2];
			return this.renderProbabilities(probabilities);
		case 1:
			var _g1 = new haxe_ds_StringMap();
			var value = doom_core__$AttributeValue_AttributeValue_$Impl_$.fromString("display:none");
			if(__map_reserved["style"] != null) {
				_g1.setReserved("style",value);
			} else {
				_g1.h["style"] = value;
			}
			var value1 = doom_core__$AttributeValue_AttributeValue_$Impl_$.fromString("no probabilities");
			if(__map_reserved["data-comment"] != null) {
				_g1.setReserved("data-comment",value1);
			} else {
				_g1.h["data-comment"] = value1;
			}
			return doom_core__$VNode_VNode_$Impl_$.el("div",_g1);
		}
	}
	...
	,mouseEnter: function(value) {
		var _gthis = this;
		return function() {
			_gthis.update({ expression : _gthis.props.expression, parsed : _gthis.props.parsed, probabilities : _gthis.props.probabilities, selected : haxe_ds_Option.Some(value)},{ fileName : "ProbabilitiesView.hx", lineNumber : 142, className : "view.ProbabilitiesView", methodName : "mouseEnter"});
		};
	}
...
view_Editable.range = { start : 100, end : 100};
...
view_ProbabilitiesView.worker = (function($this) {
	...
	worker.onmessage = function(e) {
		...
		if(null != view_ProbabilitiesView.view && expr == view_ProbabilitiesView.view.props.expression) {
			view_ProbabilitiesView.view.update({ expression : view_ProbabilitiesView.view.props.expression, parsed : view_ProbabilitiesView.view.props.parsed, probabilities : haxe_ds_Option.Some(p), selected : view_ProbabilitiesView.view.props.selected},{ fileName : "ProbabilitiesView.hx", lineNumber : 24, className : "view.ProbabilitiesView", methodName : "worker"});
		}
	};
	var collect = { };
	{
