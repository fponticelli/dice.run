...
view_ProbabilitiesView.prototype = $extend(doom_html_Component.prototype,{
	...
	,render: function() {
		...
		var children = doom_core__$VNode_VNode_$Impl_$.el("div",_g1,doom_core__$VNodes_VNodes_$Impl_$.children([doom_core_VNodeImpl.Text(StringTools.replace(StringTools.replace(StringTools.replace(Loc.msg.probabilitiesStats,"$minValue",f(stats.minValue)),"$maxValue",f(stats.maxValue)),"$count",f(stats.count)),null,null)]));
		var _g2 = new haxe_ds_StringMap();
		...
		var children1 = doom_core__$VNode_VNode_$Impl_$.el("div",_g4,doom_core__$VNodes_VNodes_$Impl_$.children(stats.map($bind(this,this.renderAtLeast))));
		var _g5 = new haxe_ds_StringMap();
		...
		var children2 = doom_core__$VNode_VNode_$Impl_$.el("div",_g5,doom_core__$VNodes_VNodes_$Impl_$.children(stats.map($bind(this,this.renderProb))));
		...
		var children3 = doom_core__$VNode_VNode_$Impl_$.el("div",_g3,doom_core__$VNodes_VNodes_$Impl_$.children([children1,children2,doom_core__$VNode_VNode_$Impl_$.el("div",_g6,doom_core__$VNodes_VNodes_$Impl_$.children(stats.map($bind(this,this.renderAtMost))))]));
		var _g7 = new haxe_ds_StringMap();
	...
	,mouseEnter: function(value) {
		var _gthis = this;
		return function() {
			_gthis.update({ expression : _gthis.props.expression, parsed : _gthis.props.parsed, probabilities : _gthis.props.probabilities, selected : haxe_ds_Option.Some(value)},{ fileName : "ProbabilitiesView.hx", lineNumber : 94, className : "view.ProbabilitiesView", methodName : "mouseEnter"});
		};
	}
...
Loc.msg = { expectedOneOf : "expected one of", valueOrMore : "$value or more", valueOrLess : "$value or less", validationPrefix : ["the expression is valid but the numbers are wrong!","you need to be more serious ...","are you being serious?","let's not be silly, shall we?","are you for real?","enough is enough","really?","what a joker you are"], atLeast : "at least", emptySet : "invalid empty set of dice", jumpersideasLink : "https://jumpersideas.com/#!dice-roller", probabilities : "probabilities", infiniteReroll : "this will roll forever! a d$sides always matches $range", jumpersideas : "jumpersideas.com", endOfFile : "end of file", tooManyDice : "Too many dice to display", found : "found", or : "or", exact : "exactly $value", probabilitiesStats : "values between $minValue and $maxValue, samples: $count", between : "between $minInclusive and $maxInclusive", dropOrKeepShouldBePositive : "a drop or keep value should always be more than zero", tooManyDrops : "the set has $available value(s) and you are dropping $toDrop?", tooManyKeeps : "the set has $available value(s) and you are keeping $toKeep?", useSeed : "use seed", asConfabulatedOn : "as confabulated on", insufficientSides : "a d$sides is not a valid die", atMost : "at most"};
